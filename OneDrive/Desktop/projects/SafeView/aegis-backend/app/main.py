...
audio_service = AudioService(model_name="base")
...
async def health_check() -> dict:
    ...
    return {
        ...
        "components": {
            "api": "operational",
                "vision_service": "operational (yolov8)",
        }
    }
...
async def analyze_image(
    ...
    try:
        ...
        for det in results:
            ...
            result_str = "BLOCK" if det.label.startswith("Violence") or det.label.startswith("Nudity") else "ALLOW"
...
async def analyze_media_metadata(request: MetadataRequest) -> dict:
    ...
    """
    try:
        result = await analyze_metadata(request.title, request.blocked_themes)
        # Presentation log: metadata themes and decision
        res_status = result.get("status", "ALLOW")
        reason = result.get("reason", "")
        logger.info(f"Metadata - {res_status} - title={request.title}, reason={reason}")
        return result
    except Exception as e:
        raise HTTPException(
            status_code=500,
...
async def analyze_audio(
    ...
    try:
        ...
        if not filter_profanity:
            profanity = {"contains_profanity": False, "action": "ALLOW", "duration": 0.0, "matched": []}
        else:
            profanity = analyze_profanity(text or "")
        # Presentation log: audio transcript and action
        action = profanity.get("action", "ALLOW")
        logger.info(f"Audio - {('BLOCK' if action=='MUTE' else 'ALLOW')} - text={text[:120]}")
        return {"transcribed_text": text, "profanity_analysis": profanity}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(
